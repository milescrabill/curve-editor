...
#ifndef curvePractical_beziernumber_h
#define curvePractical_beziernumber_h
...
public:
    ...
    float2 getPoint(float t) {
        ...
        for (int i = 0; i < controls.size(); i++) {
            double b = bernstein(i, int(controls.size()) - 1, t);
            double bx = controls[i].x * b;
            double by = controls[i].y * b;
            r.x += bx;
            r.y += by;
        }
        return r;
    ...
    float2 getDerivative(float t) {
        float2 r(0.0f, 0.0f);
        for (int i = 0; i < controls.size(); i++) {
            double db = bernsteinDerivative(i, int(controls.size()) - 1, t);
            double dbx = controls[i].x * db;
            double dby = controls[i].y * db;
            r.x += dbx;
            r.y += dby;
        }
        return r;
